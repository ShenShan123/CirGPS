...
# added by shan
from graphgps.loader.dataset.sram_dataset import SealSramDataset

def log_loaded_dataset(dataset, format, name):
    logging.info(f"[*] Loaded dataset '{name}' from '{format}':")
    logging.info(f"  num tasks: {dataset.task_type}")
    logging.info(f"  {dataset._data}")
    logging.info(f"  undirected: {dataset[0].is_undirected()}")
    logging.info(f"  num graphs: {len(dataset)}")
    logging.info(f"  num hops: {dataset.num_hops}")
    total_num_nodes = 0
    total_num_edges = 0
    ...
    logging.info(f"  total num nodes: {total_num_nodes}")
    if hasattr(dataset.data, 'num_edges'):
        total_num_edges = dataset.data.num_edges
        logging.info(f"  total num edges: {total_num_edges}")
    logging.info(f"  avg num_nodes/graph: "
                 f"{total_num_nodes // len(dataset)}")
    logging.info(f"  avg num_edges/graph: "
                 f"{total_num_edges // len(dataset)}")
...
def load_dataset_master(format, name, dataset_dir):
    ...
    # GraphGym default loader for Pytorch Geometric datasets
        ...
        dataset = performat_SramDataset(
            name=name, 
            dataset_dir='/data1/shenshan/dgl/GraphGPS/datasets/',
            num_hops=cfg.dataset.num_hops,
            add_target_edges=cfg.dataset.add_target_edges,
            ratio_per_hop=cfg.dataset.ratio_per_hop,
            neg_edge_ratio=cfg.dataset.edge_negative_sampling_ratio,
            to_undirected=cfg.dataset.to_undirected,
            shuffle_split=cfg.dataset.shuffle_split,
            num_sampler=cfg.dataset.num_sampler,
            sample_rates=cfg.dataset.sample_rate,
        ...
        logging.info(f"Building dataset {name} took {timestr}")
    else:
        raise ValueError(f"Unknown data format: {format}")

    # we comment out this func for performance, and we have no preprocessing need
    """
    pre_transform_in_memory(
        dataset, partial(task_specific_preprocessing, cfg=cfg), show_progress=True
    )
    ...
    # adding expander edges, merge from Exphormer
    if cfg.prep.exp: pe_enabled_list.append('exp')
    if cfg.prep.dist_enable: pe_enabled_list.append('dist')
    
    start = time.perf_counter()

    if format == "SRAM":
        # if we have cached the pe encodings, 
        # just load them and merge into self.data, added by shan
        dataset_pe_enabled_list = dataset.has_processed_pe(pe_enabled_list)
    
    if format == "SRAM" and dataset_pe_enabled_list:
        ...
        pre_encoding_in_memory(
            dataset,
            data_pe_tuple=dataset_pe_enabled_list,
            cfg=cfg,
        )
        check_pe_in_dataset(dataset, pe_enabled_list)
    ...
    timestr = time.strftime('%H:%M:%S', time.gmtime(elapsed)) \
                + f'{elapsed:.2f}'[-3:]
    logging.info(f"Graph encoding took {timestr}")
    
    # move logger here! by shan
    log_loaded_dataset(dataset, format, name)
...
def performat_SramDataset(dataset_dir, name, num_hops, 
                          add_target_edges, ratio_per_hop, 
                          neg_edge_ratio, to_undirected, 
                          shuffle_split, num_sampler,
                          sample_rates, task_type):
    dataset = SealSramDataset(
        name=name, root=dataset_dir, num_hops=num_hops,
        add_target_edges=add_target_edges,
        # pre_transform=shortest_path_to_src_dst
        ratio_per_hop=ratio_per_hop,
        neg_edge_ratio=neg_edge_ratio,
        to_undirected=to_undirected,
        shuffle_split=shuffle_split,
        num_sampler=num_sampler,
        sample_rates=sample_rates,
        task_type=task_type,
    )
    s_dict = dataset.get_idx_split('shuffle')
    dataset.split_idxs = [s_dict[s] for s in ['train', 'val', 'test']]
    # logging.info("dataset.split_idxs", dataset.split_idxs)
    return dataset

def check_pe_in_dataset(dataset, pe_types):
    if dataset._data_list is not None:
        dataset.data, dataset.slices = dataset.collate(dataset._data_list)
        dataset._data_list = None

    for (data_name, start) in dataset.data_offsets.items():
        data0 = dataset[start]
        ...
        if 'ERN' in pe_types:
            if not hasattr(data0, 'er_emb'):
                raise AttributeError("data in dataset doesn't have attribute 'er_emb', using {pe_types} encoder/encoders")
        if 'ERE' in pe_types:
            if not hasattr(data0, 'er_edge'):
                raise AttributeError("data in dataset doesn't have attribute 'er_edge', using {pe_types} encoder/encoders")
        if 'exp' in pe_types:
            if not hasattr(data0, 'expander_edges'):
                raise AttributeError("data in dataset doesn't have attribute 'expander_edges', using {pe_types} encoder/encoders")
