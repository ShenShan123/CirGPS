...
class EREdgeEncoder(torch.nn.Module):
    def __init__(self, emb_dim, use_edge_attr=False, expand_edge_attr=False, expand_x=False):
        super().__init__()

        # dim_in = cfg.gt.dim_edge  # Expected final edge_dim
        dim_in = cfg.dataset.edge_dim  # Expected final edge_dim
        ...
        if not self.use_edge_attr:
            assert emb_dim == dim_in, f"emb_dim={emb_dim}, dim_in={dim_in}"
