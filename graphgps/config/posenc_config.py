...
def set_cfg_posenc(cfg):
    ...
    cfg.posenc_DRNL = CN() # added by shan
    cfg.posenc_SPD = CN() # added by shan
    cfg.posenc_CirStatis = CN() # added by shan

    # Effective Resistance Embeddings, by exphormer
    ...
    # just for DRNL PE, by shan
    pecfg = getattr(cfg, 'posenc_DRNL')
    pecfg.max_drnl = 1024
    # Size of Positional Encoding embedding
    pecfg.dim_pe = 16
    pecfg.enable = False
    # just for SPD PE, by shan
    pecfg = getattr(cfg, 'posenc_SPD')
    pecfg.max_dist = 128
    # Size of Positional Encoding embedding
    pecfg.dim_pe = 16
    pecfg.enable = False
    # just for CirStatis PE, by shan
    pecfg = getattr(cfg, 'posenc_CirStatis')
    pecfg.dim_pe = 16
    pecfg.enable = False
    
    for name in ['posenc_LapPE', 'posenc_SignNet',
                 'posenc_RWSE', 'posenc_HKdiagSE', 'posenc_ElstaticSE',
                 'posenc_ERN', 'posenc_ERE']: # by exphormer
    ...
    # Setting accuracy for Effective Resistance Calculations:, by exphormer
    cfg.posenc_ERN.accuracy = 0.1
    cfg.posenc_ERE.accuracy = 0.1

    # To be set during the calculations: by exphormer
