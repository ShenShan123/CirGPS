...
def train_epoch(logger, loader, model, optimizer, scheduler, batch_accumulation):
    ...
    for iter, batch in enumerate(tqdm(loader, ncols=100)):
        batch.split = 'train'
        ...
        if cfg.dataset.name == 'ogbg-code2':
            ...
            # add weighted_CE loss, by shan
            if cfg.model.loss_fun == 'weighted_cross_entropy':
                loss, pred_score = weighted_cross_entropy(pred.squeeze(), true.squeeze(), batch.weight)
            else:
...
def custom_train(loggers, loaders, model, optimizer, scheduler):
    ...
    for cur_epoch in range(start_epoch, cfg.optim.max_epoch):
        ...
        # Checkpoint with regular frequency (if enabled). Changed by shan
