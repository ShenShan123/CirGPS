...
class LocalModel(nn.Module):
    def __init__(self, dim_h, local_gnn_type, edge_type, edge_attr_type, num_heads,
                ...
                layer_norm=False, batch_norm=True, res_linear=False):
        super().__init__()
        ...
        self.residual_linear = nn.Linear(dim_h, dim_h)
...
class MultiLayer(nn.Module):
    ...
    def __init__(self, dim_h,
                 ...
                 bigbird_cfg=None, exp_edges_cfg=None, res_linear=False):
        super().__init__()
        ...
        # MPNN uses BN first, 
        # global atte. uses LN first, added by shan
        self.local_batch_norm = batch_norm
        self.local_layer_norm = False if batch_norm else layer_norm
        self.global_layer_norm = layer_norm
        self.global_batch_norm = False if layer_norm else batch_norm
        ...
        for layer_type in model_types:
            ...
            if layer_type in {'Transformer', 'ETransformer', 'Exphormer', 'Performer', 'BigBird'}:
                self.models.append(GlobalModel(
                    dim_h=dim_h,
                    ...
                    layer_norm=self.global_layer_norm,
                    batch_norm=self.global_batch_norm,
                ...
                self.models.append(LocalModel(
                    ...
                    # layer_norm=self.layer_norm,
                    # batch_norm=self.batch_norm,
                    layer_norm=self.local_layer_norm,
                    batch_norm=self.local_batch_norm,
                    res_linear=res_linear)
                )
            else:
        ...
        # if we have already set batch_norm, 
        # just use bn for better performance, by shan
        if self.layer_norm:
    ...
    def forward(self, batch):
        ...
        # we give BN higher priority, by shan
        if self.batch_norm:
            h = self.norm2(h)
