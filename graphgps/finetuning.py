...
def load_pretrained_model_cfg(cfg):
    ...
    # we keep dropout, normalization, residual linear configs for the new model, by shan
    drp = cfg.gt.dropout
    adrp = cfg.gt.attn_dropout
    bn = cfg.gt.batch_norm
    ln = cfg.gt.layer_norm
    res = cfg.gt.res_linear
    # Copy over GT config
    cfg.gt = pretrained_cfg.gt
    # added by shan
    cfg.gt.dropout = drp
    cfg.gt.attn_dropout = adrp
    cfg.gt.batch_norm = bn
    cfg.gt.layer_norm = ln
    cfg.gt.res_linear = res
    ...
    act = cfg.gnn.act
    drp = cfg.gnn.dropout
    cirp = cfg.gnn.cir_statis_proj # added by shan
    bn = cfg.gnn.batchnorm  # added by shan
    cfg.gnn = pretrained_cfg.gnn
    cfg.gnn.head = head
    cfg.gnn.layers_post_mp = post_mp
    cfg.gnn.act = act
    cfg.gnn.dropout = drp
    cfg.gnn.cir_statis_proj = cirp # added by shan
    cfg.gnn.batchnorm = bn  # added by shan
    return cfg

# TODO: very import function!! by shan
def init_model_from_pretrained(model, pretrained_dir,
    ...
    if reset_prediction_head:
        ...
        pretrained_dict = {k: v for k, v in pretrained_dict.items()
                           if not k.startswith('model.post_mp')}
    #     for key, v in pretrained_dict.items():
    #         print("after reset pretrained_dict key", key, v.requires_grad)
    # assert 0
